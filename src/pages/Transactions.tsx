      ...
      <div className="bg-white border style={{borderColor: 'var(--border)'}} rounded-[13px] style={{boxShadow: 'var(--shadow)'}} p-8 mt-8">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="ml-auto flex flex-wrap items-center gap-6 text-sm">
            <div className="flex items-center gap-2">
              <span className="style={{color: 'var(--text-muted)'}}">收入：</span>
              <span className="font-bold style={{color: 'var(--success)'}} text-lg number">¥{totalIncome.toLocaleString()}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="style={{color: 'var(--text-muted)'}}">支出：</span>
              <span className="font-bold style={{color: 'var(--danger)'}} text-lg number">¥{totalExpense.toLocaleString()}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="style={{color: 'var(--text-muted)'}}">结余：</span>
              <span className={`font-bold text-lg number ${totalIncome - totalExpense >= 0 ? 'style={{color: 'var(--success)'}}' : 'style={{color: 'var(--danger)'}}'}`}>
          ...
          <table className="w-full">
            <thead>
              <tr className="style={{backgroundColor: 'var(--bg-soft)'}} border-b style={{borderColor: 'var(--border)'}}">
                ...
                <th className="text-left px-5 py-3 text-xs font-bold style={{color: 'var(--text-muted)'}} uppercase tracking-wider">分类</th>
              ...
              {transactions.length === 0 ? (
                <tr>
                  <td colSpan={6} className="py-20 text-center empty-state">
                    ...
                    <p className="text-sm style={{color: 'var(--text-muted)'}} mt-1">点击右上角「记一笔」开始记录您的第一笔收支</p>
                  </td>
                </tr>
              ) : (
                    ...
                    <tr key={transaction.id} className="border-b style={{borderColor: 'var(--border)'}} hover:style={{backgroundColor: 'var(--bg-soft)'}} transition-all">
                      ...
                      <td className="px-5 py-4">
                        <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                          transaction.type === 'income' ? 'bg-green-50 style={{color: 'var(--success)'}}' : 'bg-red-50 style={{color: 'var(--danger)'}}'
                        }`}>
                      ...
                      <td className={`px-5 py-4 font-bold text-lg number ${
                        transaction.type === 'income' ? 'style={{color: 'var(--success)'}}' : 'style={{color: 'var(--danger)'}}'
                      }`}>
