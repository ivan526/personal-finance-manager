    ...
    <div className="app-shell min-h-screen grid grid-cols-[300px_1fr]">
      ...
      <aside className="sidebar border-r bg-white/82 backdrop-blur-lg p-5 sticky top-0 h-screen transition-all" style={{borderColor: 'var(--border)', boxShadow: 'var(--shadow-nav)'}}">
        ...
        <nav className="nav grid gap-2 pt-0.5">
            ...
            <Link
              ...
              className={`w-full flex items-center gap-3.25 px-4.5 py-3 rounded-[8px] transition-all h-[50px] no-underline ${
                location.pathname === item.path
                  ...
                  : 'text-[#26364d] font-medium hover:style={{backgroundColor: 'var(--bg-soft)'}} hover:style={{color: 'var(--text-secondary)'}}'
              }`}
            >
          ...
          <div className="mt-[70px] mx-2 rounded-[8px] bg-gradient-to-br from-[#eef6ff] to-[#f8fbff] border border-[#edf5ff] style={{boxShadow: 'var(--shadow-soft)'}} p-5 min-h-[150px]">
          ...
          <div className="mt-6 px-4 text-center">
            <div className="text-xs style={{color: 'var(--text-muted)'}}">
              <div className="flex items-center justify-center gap-1.5 mb-1">
                <span className="style={{color: 'var(--primary)'}} font-semibold">FinSpace</span>
                <span>© 2026</span>
              </div>
       ...
       <main id="main-content" className="min-w-0 ml-4">
         <header className="topbar sticky top-0 z-10 min-h-[75px] border-b style={{borderColor: 'var(--border)'}} bg-white flex items-center justify-between gap-4 px-8 py-3 shadow-sm">
           <div>
           ...
           <div className="actions flex items-center gap-3">
             <button
               ...
               aria-label="添加收支记录"
               className="h-[44px] px-5.5 rounded-[7px] bg-gradient-to-b from-[#1d74ff] to-[#075ff0] text-white font-bold text-base flex items-center gap-2 style={{boxShadow: 'var(--shadow-btn)'}} hover:translate-y-[-1px] transition-all min-w-[100px] md:flex hidden"
             >
       ...
       {/* 移动端底部导航 */}
       <nav className="fixed bottom-0 left-0 right-0 bg-white border-t style={{borderColor: 'var(--border)'}} z-50 md:hidden shadow-[0_-2px_10px_rgba(0,0,0,0.05)]">
