...
export default function AddTransactionModal({ onClose, editTransaction }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-[2px] z-50 flex items-center justify-center p-4 animate-fade-in">
       <div className="bg-white rounded-[16px] shadow-[0_20px_60px_rgba(15,23,42,0.15)] w-full max-w-lg overflow-hidden animate-slide-up border style={{borderColor: 'var(--border)'}} overscroll-behavior-contain modal">
        <div className="flex items-center justify-between px-8 py-6 border-b style={{borderColor: 'var(--border)'}}">
            ...
            <button
              ...
              className={`px-5 py-2.5 rounded-md font-semibold transition-all text-base ${
                recordType === 'expense_income' 
                  ? 'style={{backgroundColor: 'var(--primary)'}} text-white shadow-[0_4px_12px_rgba(29,110,255,0.18)]' 
                  : 'style={{color: 'var(--text-muted)'}} hover:style={{backgroundColor: 'var(--bg-soft)'}}'
              }`}
            >
              收支
            </button>
          ...
          <button 
            onClick={onClose} 
            aria-label="关闭弹窗"
            className="p-2.5 hover:style={{backgroundColor: 'var(--bg-soft)'}} rounded-md transition-all"
          >
            <X size={20} className="style={{color: 'var(--text-muted)'}}" aria-hidden="true" />
          </button>
        </div>
         ...
         <form onSubmit={handleSubmit} className="p-8 space-y-8 modal-content">
          {recordType === 'expense_income' ? (
                 ...
                 <button
                   ...
                   className={`flex-1 py-3.5 rounded-md font-semibold transition-all text-base ${
                     type === 'expense'
                       ...
                       : 'style={{backgroundColor: 'var(--bg-soft)'}} style={{color: 'var(--text-muted)'}} hover:bg-[var(--border)]'
                    }`}
                 >
                   支出
                 </button>
           ...
           {/* 分类选择 */}
           <div>
             <label className="block text-base font-semibold style={{color: 'var(--text)'}} mb-4">选择分类</label>
                 ...
                 <button
                   ...
                   className={`py-4 rounded-[10px] text-center transition-all border ${
                     selectedCategory === category.id
                       ...
                       : 'style={{borderColor: 'var(--border)'}} hover:border-blue-200 hover:bg-blue-50/50'
                    }`}
                 >
           ...
           {/* 金额 */}
           <div>
             <label className="block text-base font-semibold style={{color: 'var(--text)'}} mb-3">金额</label>
             <input
               type="number"
               step="0.01"
               value={amount}
           ...
           {/* 时间 */}
           <div>
             <label className="block text-base font-semibold style={{color: 'var(--text)'}} mb-3">时间</label>
             <input
               type="datetime-local"
               value={time}
           ...
           {/* 备注 */}
           <div>
             <label className="block text-base font-semibold style={{color: 'var(--text)'}} mb-3">备注（可选）</label>
             <input
               type="text"
               value={remark}
             ...
             />
              ...
              <div className="text-center py-12">
                <div className="w-20 h-20 bg-gradient-to-br from-blue-50 to-blue-100 rounded-[16px] grid place-items-center mx-auto mb-6">
                  <TrendingUp size={32} className="style={{color: 'var(--primary)'}}" aria-hidden="true" />
                </div>
                <h4 className="text-xl font-bold style={{color: 'var(--text)'}} mb-3">投资交易记录</h4>
                <p className="style={{color: 'var(--text-muted)'}} mb-6">投资交易需要更多信息，请前往投资页面记录</p>
                <Link
                  to="/investment"
                  onClick={onClose}
